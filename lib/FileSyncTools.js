...
_.extend(FileSyncTool.prototype, {
	...
	listMissingFiles: function (path1, path2, complete){
		_compareFiles(path1, path2, true, this, complete);
	},
	listDuplicateFiles: function (path1, path2, complete) {
		debug('searching for duplicates');
		_compareFiles(path1, path2, false, this, complete);
	}
});

// --- Private Helper functions ---
function _compareFiles (path1, path2, missing, fileSync, complete) {
	...
	fileSync.listFilesRecursive(path1, function (err, files) {
		if (err) return complete(err);
		files1 = files;
		_processedList();
	});
	fileSync.listFilesRecursive(path2, function (err, files) {
		if (err) return complete(err);
		files2 = files;
		_processedList();
	});
