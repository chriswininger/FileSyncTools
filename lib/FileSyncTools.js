...
function _compareFiles (path1, path2, missing, fileSync, options, complete) {
    ...
    var compareHash = options.includeHash;
    options.includeHash = false;

	var _processedList = _.after(2, function () {
        _findDiffOrOverlap(files1, files2, missing, compareHash, complete);
	});
 ...
 * @private
 */
function _findDiffOrOverlap(files1, files2, missing, compareHash, complete) {
	var diff = [];
    ...
    function _compFile (file, _complete) {
        var copy = _.find(files2, function (f) { return file.fileName === f.fileName; });
