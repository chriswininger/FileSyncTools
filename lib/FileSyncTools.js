...
var fs = require('fs'),
	...
	debug = require('debug')('FileSyncTools:Lib'),
	cp = require('cp'),
	Path = require('path'),
...
var DEFAULT_EXCLUSIONS = ['.DS_Store'];

function FileSyncTool () {
    this.exclusionList = DEFAULT_EXCLUSIONS;
}
_.extend(FileSyncTool.prototype, {
	...
	listMissingFiles: function (path1, path2, options, complete){
        options.excluded = this.exclusionList;
...
function _findDiffOrOverlap(files1, files2, missing, options, complete) {
    ...
    function _compFile (file, _complete) {
        ...
        if (missing) {
            if (!copy) {
                ...
                if (!options.includeHash) return _complete();
    ...
    function _addFile (file) {
        ...
        if (_isExcluded(file.fileName)) return;
    ...
    function _isExcluded (fName) {
        if (!options.excluded) return false;
        return _.contains(options.excluded, fName);
    }
}
